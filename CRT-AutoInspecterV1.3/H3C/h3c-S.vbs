...
Sub Main
    ...
    CMDArr(1) = "display cpu"
    CMDArr(2) = "display memory"
    ...
    CMDArr(9) = "display device manuinfo"

    crt.Screen.Clear()
    crt.Screen.Send vbcrlf
    crt.Screen.WaitForString(Dprompt)
    crt.Screen.Synchronous = True
    crt.Screen.Send "screen-length disable" & vbcrlf
    crt.Screen.WaitForString(Dprompt)
    crt.Screen.Send vbcrlf
    crt.Screen.WaitForString(Dprompt)
    ...
    crt.Screen.WaitForString(Dprompt)
    crt.Sleep 1000
    crt.Screen.Send "display diagnostic-information" & chr(13)
    index = crt.Screen.WaitForStrings(" This operation may take a few minutes, continue?[Y/N]"," Diagnostic-information is saved to Flash or displayed(Y=save N=display)?[Y/N]",3)
    select case index
        case 1
            crt.Screen.Send "y" & chr(13)
            if(crt.Screen.WaitForString(" Diagnostic-information is saved to Flash or displayed(Y=save N=display)?[Y/N]",2)) Then
